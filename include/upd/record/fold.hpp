...
constexpr auto fold_left = []<record_like Record>(Record &&rec,
                                                  auto &&init,
  ...
  return UPD_WITH_SEQUENCE(Is, record_size_v<Record>, &) {
    return (UPD_FWD(init), ...,
            variadic::folder(get_ith_entry<Is>(UPD_FWD(rec)), f));
  };
};
...
constexpr auto fold_right =
  ...
  return UPD_WITH_SEQUENCE(Is, record_size_v<Record>, &) {
    return (variadic::folder(get_ith_entry<Is>(UPD_FWD(rec)), f), ...,
            UPD_FWD(init));
  };
};
