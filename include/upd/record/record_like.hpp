...
concept ith_record_tag_gettable =
    requires(Record rec) { record_tag<I, Record>::value; }
    && requires(Record rec,
                record_tag<I, Record>
                    tag) { typename record_element<tag.value, Record>::type; }
    && requires(Record rec,
                record_tag<I, Record> tag,
                record_element<tag.value, Record> elem) {
         {
...
concept record_like =
    requires(Record rec) {
      ...
      } -> std::equality_comparable_with<std::size_t>;
    }
    && UPD_ALL_OF_CONCEPT(
        ith_record_tag_gettable, Record, record_size<Record>::value);
...
struct record_size<Record> {
  constexpr static auto value =
      record_like_for<std::remove_cvref_t<Record>>::size;
};
...
struct record_tag<I, Record> {
  constexpr static auto value =
      record_like_for<std::remove_cvref_t<Record>>::template tag<I>;
};

template<auto Tag, typename Record>
  requires implementation_of<Record, record_like_for>
struct record_element<Tag, Record> {
  using type = typename record_like_for<
      std::remove_cvref_t<Record>>::template element_type<Tag>;
};
