...
struct lite_record_tag_node {
  ...
  has_tag(auto_constant<Tag>) noexcept(release) -> bool {
    return true;
  }
};
...
struct lite_record_type_node {
  ...
  [[nodiscard]] constexpr static auto
  has_type(typebox<T>) noexcept(release) -> bool {
    return true;
  }
};
...
struct lite_record_node : lite_record_tag_node<Tag>, lite_record_type_node<T> {
  ...
  get_by_tag(auto_constant<Tag>) & noexcept(release) -> T & {
    return value;
  }

  [[nodiscard]] constexpr auto
  get_by_tag(auto_constant<Tag>) const & noexcept(release) -> const T & {
    return value;
  }
  ...
  [[nodiscard]] constexpr static auto
  get_type_by_tag(auto_constant<Tag>) noexcept(release) -> typebox<T> {
    return typebox<T>{};
  }

  [[nodiscard]] constexpr auto
  find_by_type(typebox<T>) const noexcept(release) -> tag_type {
    return tag;
  }
...
struct lite_record<lite_record_node<Tags, Ts>...>
    : lite_record_node<Tags, Ts>... {
  ...
  explicit constexpr lite_record(Nodes &&...nodes)
      : lite_record_node<Tags, Ts>{UPD_FWD(nodes)}... {}

  template<auto Tag>
  constexpr static void get_by_tag(auto_constant<Tag>) noexcept(release) {
    UPD_STATIC_ASSERT(always_false<>, "Lite record does not contain '{}' tag",
                      Tag);
  }
  ...
  constexpr static void find_by_type(typebox<T>) noexcept(release) {
    UPD_STATIC_ASSERT(always_false<>,
                      "Lite record does not contain any '{}' value", typeid(T));
  }
...
struct record_element<Tag, lite_record<Nodes...>> {
  using type = typename decltype(lite_record<Nodes...>::get_type_by_tag(
      expr<Tag>))::type;
};
...
[[nodiscard]] constexpr static auto collect(Record &&rec) {
  return UPD_WITH_SEQUENCE(Is, record_size_v<Record>, &) {
    return lite_record{lite_record_node{expr<record_tag_v<Is, Record>>,
...
template<auto Tag, typename... Nodes>
[[nodiscard]] constexpr static auto
has_tag(const lite_record<Nodes...> &rec) noexcept(release) -> bool {
  return rec.has_tag(expr<Tag>);
}

template<auto Tag, typename... Nodes>
[[nodiscard]] constexpr static auto
has_type(const lite_record<Nodes...> &rec) noexcept(release) -> bool {
