...
[[nodiscard]] constexpr inline auto operator==(const name<N> &lhs, const name<M> &rhs) noexcept(release) -> bool {
  return std::string_view{lhs.string} == std::string_view{rhs.string};
};

template<std::size_t N>
[[nodiscard]] constexpr inline auto operator==(const name<N> &lhs, std::string_view rhs) noexcept(release) -> bool {
  return std::string_view{lhs.string} == rhs;
};

template<std::size_t N>
[[nodiscard]] constexpr inline auto operator==(std::string_view lhs, const name<N> &rhs) noexcept(release) -> bool {
...
struct std::formatter<upd::name<N>> {
  ...
  [[nodiscard]] constexpr static auto format(const upd::name<N> &nm, std::format_context &ctx) {
    auto it = ctx.out();
