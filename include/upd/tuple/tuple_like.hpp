...
concept ith_tuple_element_gettable =
    requires(std::remove_reference_t<Tuple> x) {
...
struct std::tuple_size<Tuple> {
  constexpr static auto value =
      upd::tuple_like_for<std::remove_cvref_t<Tuple>>::size;
};

template<std::size_t I, typename Tuple>
  requires upd::implementation_of<Tuple, upd::tuple_like_for>
struct std::tuple_element<I, Tuple> {
  using type = typename upd::tuple_like_for<
      std::remove_cvref_t<Tuple>>::template element_type<I>;
};
