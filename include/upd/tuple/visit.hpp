...
constexpr auto visit = []<tuple_like2 Tuple, typename F> [[nodiscard]] (
                           Tuple &&t, std::size_t i, F &&f) -> decltype(auto) {
  ...
  using retval_type = decltype(apply_type(
      xxx{},
      []<typename... Metatype> -> std::common_type_t<
                                   typename Metatype::type...> {}));

  UPD_ASSERT(i < tuple_size_v<Tuple>);

  auto make_invoker_for_ith = []<std::size_t I>(expr_t<I>) {
    return +[](Tuple &&t, F &&f) -> retval_type {
      return UPD_INVOKE(UPD_FWD(f), get<I>(UPD_FWD(t)));
    };
  };

  auto lut = UPD_WITH_SEQUENCE(Is, tuple_size_v<Tuple>, &) {
