...
struct variable {
  ...
  operator==(variable<Name>) const noexcept(release) -> bool {
    return true;
  }

  template<auto VN>
  [[nodiscard]] constexpr auto
  operator==(variable<VN>) const noexcept(release) -> bool {
    return false;
  }
};
...
[[nodiscard]] constexpr auto
substitute(variable<Varname>, const Lets &lets) noexcept(release) {
...
[[nodiscard]] constexpr auto
balance_on(const Expr &base, variable<Varname>) noexcept(release) -> Expr {
  return base;
}

template<auto Varname>
[[nodiscard]] constexpr auto
dependencies(variable<Varname> var) noexcept(release) {
