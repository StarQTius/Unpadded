...
struct multiply {
  ...
  operator==(const multiply &) const noexcept(release) -> bool = default;
};

template<expression Lhs, expression Rhs, record_like Lets>
[[nodiscard]] constexpr auto
substitute(const multiply<Lhs, Rhs> &expr, const Lets &lets) noexcept(release) {
...
[[nodiscard]] constexpr auto
balance_on(const Expr &base, const multiply<Lhs, Rhs> &expr) noexcept(release) {
  UPD_STATIC_ASSERT(
...
template<expression Lhs, expression Rhs>
[[nodiscard]] constexpr auto
dependencies(const multiply<Lhs, Rhs> &expr) noexcept(release) {
...
struct divide {
  ...
  [[nodiscard]] constexpr auto
  operator==(const divide &) const noexcept(release) -> bool = default;
};

template<expression Lhs, expression Rhs, record_like Lets>
[[nodiscard]] constexpr auto
substitute(const divide<Lhs, Rhs> &expr, const Lets &lets) noexcept(release) {
  return substitute(expr.lhs, lets) / substitute(expr.rhs, lets);
}
...
template<auto Varname, expression Expr, expression Lhs, expression Rhs>
  requires balanceable<Lhs, Varname> || balanceable<Rhs, Varname>
[[nodiscard]] constexpr auto
balance_on(const Expr &base, const divide<Lhs, Rhs> &expr) noexcept(release) {
  UPD_STATIC_ASSERT(
      (!depends_on_v<decltype(expr.lhs), Varname>
       || !depends_on_v<decltype(expr.rhs), Varname>),
      "{} and {} both depend on {}; Only one operand should depend on {}",
...
template<expression Lhs, expression Rhs>
[[nodiscard]] constexpr auto
dependencies(const divide<Lhs, Rhs> &expr) noexcept(release) {
  namespace updv = upd::tuple_views;
  return updv::concat(dependencies(expr.lhs), dependencies(expr.rhs))
         | updv::to<std::tuple>;
}
...
struct std::formatter<upd::algebra::multiply<Lhs, Rhs>> {
  ...
  static auto
  format(const upd::algebra::multiply<Lhs, Rhs> &eq, std::format_context &ctx) {
    auto it = ctx.out();
...
struct std::formatter<upd::algebra::divide<Lhs, Rhs>> {
  ...
  static auto
  format(const upd::algebra::divide<Lhs, Rhs> &eq, std::format_context &ctx) {
    auto it = ctx.out();
